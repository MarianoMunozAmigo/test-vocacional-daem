...
async function generarPDFResultado(
  ...
  try {
    ...
    doc.text(
      `Científico Humanista: ${porcentajeCH}%`,
      20,
      138
    );

    doc.text(
      `Técnico Profesional: ${porcentajeTP}%`,
      20,
      148
    );
    ...
    const nombreArchivo =
      `resultado-test-vocacional-${rut}.pdf`;
    ...
    alert(
      "El resultado fue guardado, pero no se pudo generar el PDF."
    );
  }
}
...
rutInput.addEventListener(
  ...
  function () {
    ...
    const rut =
      rutInput.value.trim().toUpperCase();
      ...
      for (let i = 1; i <= 15; i++) {
        ...
        puntajeCH += parseInt(
          document.querySelector(
            `input[name="ch${i}"]:checked`
          ).value
        );

        puntajeTP += parseInt(
          document.querySelector(
            `input[name="tp${i}"]:checked`
          ).value
        );
      }

      const porcentajeCH =
        Math.round((puntajeCH / 30) * 100);

      const porcentajeTP =
        Math.round((puntajeTP / 30) * 100);
